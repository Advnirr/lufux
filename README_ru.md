...
* **Проверка скорости накопителя:** Перед развёртыванием Windows To Go замеряется последовательная запись и случайная запись блоками 4 КБ. Windows To Go *работает* с накопителя, поэтому медленная флешка даёт не медленную запись, а неработоспособную систему; Lufux показывает измеренные числа и предлагает отменить запись до того, как она займёт часы.
